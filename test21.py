...
class FitnessAdvisor:
    ...
    def _convert_plan_to_markdown_enhanced(self, plan_json: Dict, profile: Dict) -> str:
        ...
        def format_exercise_block(exercise_data: Dict, index: int, section_type: str) -> str:
            ...
            # --- FORMATTING FIX 1: Add "Benefit:" label and ensure new line ---
            output += f"**Benefit:** {benefit}\n\n"
            # --- END FORMATTING FIX 1 ---
    ...
    def _build_system_prompt(
        ...
        if fitness_level in ["Level 1 – Assisted / Low Function", "Level 2 – Beginner Functional"] or age >= 60 or bmi > 30:
             if '-' in target_reps:
...
def display_interactive_workout_day(day_name: str, plan_json: Dict, profile: Dict, advisor: FitnessAdvisor):
    ...
    def render_section(section_data: List[Dict], section_title: str, section_key: str):
        ...
        for idx, exercise in enumerate(section_data):
            ...
            # --- FIX 1: Benefit display format ---
            st.markdown(f"**Benefit:** *{exercise.get('benefit', 'N/A')}*")
            # --- END FIX 1 ---
            ...
            # --- FIX 2: Explicitly set button text to '+' and '-' ---
    ...
    # =================================================================================
        ...
        for idx, day in enumerate(profile.get('days_per_week', [])):
            if day in st.session_state.all_json_plans and st.session_state.all_json_plans[day]:
                ...
                # --- APPLYING THE WORKOUT CATEGORY TO THE EXPANDER TITLE ---
                workout_category = plan_json.get('main_workout_category', 'Workout')
                expander_title = f"📋 {day} Workout Log - {workout_category}"
                
                with st.expander(expander_title, expanded=True if idx == 0 else False):
