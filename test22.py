...
# Low (MET 3.0), Moderate (MET 5.0), High (MET 8.0)
BASE_CAL_PER_MIN_70KG = {
    "Low": 5.0,  # e.g., Stretching, Walking
    "Moderate": 8.0, # e.g., Strength training, Light cardio
    "High": 13.0  # e.g., HIIT, Circuit training, Jogging
}

def _get_cal_reference(weight_kg: float) -> Dict[str, str]:
    """Calculates plausible Cal/min and Cal/rep ranges based on user weight for the LLM prompt."""
    
    # Scaling factor relative to 70kg reference
    scaling_factor = weight_kg / 70.0
    
    # Calculate Calorie/Minute (Cal/min) ranges
    cal_per_min = {
    ...
    # Round to plausible integer range (e.g., 8.2 -> 8-9)
    def format_range(value):
        low = int(max(1, value * 0.9)) # -10%
        high = int(value * 1.1)        # +10%
        return f"{low}-{high}"
        
    # Calculate Calorie/Rep (Cal/rep) as a proxy: assume 12 reps take 60 seconds (5 seconds/rep)
    # Cal/rep is calculated only for Moderate/High intensity to guide main workout
    cal_per_rep_mod = cal_per_min["Moderate"] * (5 / 60) 
    cal_per_rep_high = cal_per_min["High"] * (5 / 60)
    
    # Round Cal/rep to 1 decimal point for LLM clarity
    cal_per_rep_range = {
        "Moderate": f"{max(0.5, round(cal_per_rep_mod * 0.9, 1))}-{round(cal_per_rep_mod * 1.1, 1)}",
        "High": f"{round(cal_per_rep_high * 0.9, 1)}-{round(cal_per_rep_high * 1.1, 1)}",
    }

    return {
        "Low_Cal_Min": format_range(cal_per_min["Low"]),
        "Moderate_Cal_Min": format_range(cal_per_min["Moderate"]),
        "High_Cal_Min": format_range(cal_per_min["High"]),
        "Moderate_Cal_Rep_Range": cal_per_rep_range["Moderate"],
        "High_Cal_Rep_Range": cal_per_rep_range["High"],
...
class FitnessAdvisor:
    ...
    def _calculate_calorie_rate(self, exercise_name: str, weight_kg: float) -> tuple[float, str]:
        ...
        and provides the planned calorie rate per unit.
        """
        ...
        # Return placeholder rate (1.0) as the real rate will be calculated later in _convert_plan_to_markdown_enhanced
    ...
    def _convert_plan_to_markdown_enhanced(self, plan_json: Dict, profile: Dict) -> str:
        ...
        def format_exercise_block(exercise_data: Dict, index: int, section_type: str) -> str:
            ...
            # --- CALORIE CALCULATION (LLM-BASED) ---
            # Parse the LLM's estimated calorie output
            estimated_calories_per_exercise = parse_llm_calories(
                exercise_data.get('est_calories', 'Est: 0 Cal')
            )
        ...
        # NOTE: This summary now uses the aggregated estimated total from the LLM-derived calculation.
    ...
    def _build_system_prompt(
        ...
        """
        [MODIFIED] Builds the entire system prompt, incorporating strict calorie reference.
        """
        ...
        target_body_parts = user_profile.get("target_body_parts", ["Full Body"]) # NEW: Get target body parts
        ...
        calorie_ref = _get_cal_reference(weight_kg)
        
        calorie_strict_guide = f"""
- **User Weight:** {calorie_ref['Weight_KG']} kg
- **Low Intensity (RPE 1-4, Warmup/Cooldown):** Calorie burn is approximately **{calorie_ref['Low_Cal_Min']} Cal per minute**.
- **Moderate Intensity (RPE 5-7, Main Workout):** Calorie burn is approximately **{calorie_ref['Moderate_Cal_Min']} Cal per minute** OR **{calorie_ref['Moderate_Cal_Rep_Range']} Cal per rep** (assuming 5s/rep, 12 reps/min).
- **High Intensity (RPE 8-9, Cardio/Intervals):** Calorie burn is approximately **{calorie_ref['High_Cal_Min']} Cal per minute** OR **{calorie_ref['High_Cal_Rep_Range']} Cal per rep**.
- **CALORIE MANDATE:** The 'est_calories' value MUST be a plausible, rounded integer based on the exercise type, duration (reps/hold), and the reference rates above. Ensure the total calories for the whole session are realistic (e.g., a 30-minute Moderate session should yield roughly 240 Calories total).
"""
        ...
        prompt_parts = [
                ...
                "warmup": [
                    {
                        ...
                        "equipment": "string",
                        "safety_cue": "string",
                        "est_calories": "Est: X Cal (CRITICAL: You must provide a plausible integer calorie burn estimate for this exercise, based on the planned intensity, duration, and the user's profile weight/BMI.)" 
                    }
                ],
                "main_workout": [
                    {
            ...
            f"- **Warmup Structure Mandate (CRITICAL VARIATION):** The 3 exercises MUST follow this order and focus. Exercise names MUST be varied across different training days (e.g., use Cat-Cow Stretch, Seated Glute Stretch, or Wall Chest Stretch instead of generic 'Stretch'). **DO NOT use Arm Circles, Standing Hip Swings, or Low-Impact High Knees if a previous plan exists.**",
            "   1. Cardio Type Exercise (e.g., Low-Impact High Knees, Jumping Jacks). This exercise MUST account for 90 seconds (1.5 minutes) of the total duration.",
            "   2. Upper Body Dynamic Stretch/Mobility (e.g., Arm Circles, Band Pull-Aparts).",
            "   3. Lower Body Dynamic Stretch/Mobility (e.g., Leg Swings, Standing Hip Circles).",
            f"- Cooldown: exactly 3 exercises. Exercise names MUST be varied across different training days. **DO NOT use Seated Glute Stretch, Wall Chest Stretch, or Deep Diaphragmatic Breathing if a previous plan exists.**", # Added Cooldown variation rule
            ...
            calorie_strict_guide, # <-- INJECTED STRICT CALORIE REFERENCE GUIDE
            f"- **STRICT CALORIE PROXY RULES (Must use this structure):** For accurate logging, adhere strictly to these rules:",
            f"  - **Warmup Cardio (Exercise 1):** The planned duration is FIXED at 90 seconds. Your 'reps' field should indicate a large rep count (e.g., 60-120) to maintain LLM compliance, but the underlying time is 90s.",
            f"  - **Time/Hold Exercises (Plank/Stretches):** 'reps' field MUST be 'X seconds (or max hold)' or 'Y minutes'. The core unit is time.",
    ...
    def generate_workout_plan(
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                # the accurate LLM-calculated calorie estimate and planned units/sets into the plan_json structure.
    ...
    def _generate_fallback_plan_json(self, user_profile: Dict, day_name: str, day_focus: str, sets: str, reps: str, rest: str) -> Dict:
        ...
        cooldown = [
            ...
            {"name": "Wall Chest Stretch", "benefit": "Opens the chest and improves shoulder posture.", "steps": generic_steps, "sets": "1", "hold": f"{current_level_hold} per arm", "intensity_rpe": "RPE 1-3", "rest": "15 seconds", "equipment": "Wall", "safety_cue": "Gently rotate away from the wall; avoid straining the shoulder capsule.", "est_calories": "Est: 5 Cal"}, 
...
def calculate_performance_calorie_burn(exercise_index: str, day_name: str, advisor: FitnessAdvisor, weight_kg: float) -> float:
    ...
    # Calorie number comes DIRECTLY from the LLM's parsed value.
