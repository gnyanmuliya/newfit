...
# These values are no longer used for calorie calculation but retained for reference if needed
EXERCISE_MET_MAP = {
...
class FitnessAdvisor:
    ...
    def _convert_plan_to_markdown_enhanced(self, plan_json: Dict, profile: Dict) -> str:
        ...
        and extracts the calorie estimate from the JSON to save it into the plan_json for later use.
        """
        if not plan_json:
        ...
        def format_exercise_block(exercise_data: Dict, index: int, section_type: str) -> str:
            ...
            # --- CALORIE BURN EXTRACTION (LLM-PROVIDED) ---
            # Extract the LLM-provided estimated calorie string which is now MANDATORY 
            # in the LLM response JSON structure.
            calorie_burn_str = exercise_data.get('est_calories', 'Est: 15 Cal')
            
            # Attempt to extract the number for summation
            cal_match = re.search(r'Est:\s*(\d+)\s*Cal', calorie_burn_str)
            estimated_calories_per_exercise = 0
            if cal_match:
                estimated_calories_per_exercise = int(cal_match.group(1))
                total_calories_burned += estimated_calories_per_exercise
                
            # Store the estimated calories and planned sets/units in the plan_json
            # This is CRITICAL for the real-time calculation later.
            try:
                # Find the number of planned sets
        ...
        # NOTE: This summary now uses the aggregated estimated total from the LLM responses.
    ...
    def _build_system_prompt(
        ...
        # [MODIFIED] Added 'est_calories' field to the JSON schema. LLM must now provide the estimate.
        prompt_parts = [
                ...
                "warmup": [
                    {
                        ...
                        "equipment": "string",
                        "safety_cue": "string",
                        "est_calories": "Est: X Cal (X is a number representing the estimated total calorie burn for this exercise/set, calculated using a standard MET formula and user data)" # NEW FIELD
                    }
                ],
                "main_workout": [
                    {
    ...
    def generate_workout_plan(
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                # the LLM's calorie estimate and planned sets into the plan_json structure.
        ...
        base_exercises = [
            {
                ...
                "est_calories": "Est: 15 Cal" # FALLBACK CALORIE ESTIMATE
            },
            {
                ...
                "est_calories": "Est: 20 Cal" # FALLBACK CALORIE ESTIMATE
            },
            {
                ...
                "est_calories": "Est: 25 Cal" # FALLBACK CALORIE ESTIMATE
            },
            {
                ...
                "est_calories": "Est: 10 Cal" # FALLBACK CALORIE ESTIMATE
            },
             {
...
def calculate_performance_calorie_burn(exercise_index: str, day_name: str, advisor: FitnessAdvisor, weight_kg: float) -> float:
    ...
    the LLM's estimated calories, broken down per planned set.
    """
    ...
    # Retrieve stored LLM estimate and planned sets (stored during _convert_plan_to_markdown_enhanced)
    planned_total_cal = ex_data.get('planned_total_cal', 0)
    planned_sets = ex_data.get('planned_sets', 1)
    
    if planned_total_cal <= 0 or planned_sets <= 0:
        return 0.0

    # Calculate calorie per planned set (the breakdown)
    cal_per_planned_set = planned_total_cal / planned_sets
    
    # Calculate performance-based burn
    total_calories = cal_per_planned_set * actual_sets
    ...
    def render_section(section_data: List[Dict], section_title: str, section_key: str):
        ...
        for idx, exercise in enumerate(section_data):
            ...
            if new_units != current_units:
                ...
                # The actual_reps log is kept for completeness, but not strictly used for calorie calculation anymore.

            # --- Calorie Burn Display ---
            total_cal = calculate_performance_calorie_burn(ex_id, day_name, advisor, weight_kg)
            
            # Calculate Rate per set for display only
            planned_cal_per_set = exercise.get('planned_total_cal', 0) / exercise.get('planned_sets', 1)
            
            col_log_cal.markdown(f"**🔥 Performance Burn**")
            col_log_cal.info(f"**{round(total_cal)} Cal** (Rate Est: {planned_cal_per_set:.2f} Cal/Set)")
