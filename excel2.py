    ...
    for day_index, (day, rule) in enumerate(zip(training_days, split_rules)):
        ...
        if rule == "Active Recovery":
            ...
            for i, cat in enumerate(categories_to_select):
                cat_count = target_per_cat + (1 if i < remainder else 0)
                
                cat_pool = available_pool[available_pool['Category'].str.contains(cat, case=False, na=False) |
                                          available_pool['Category'].str.contains('Functional Strength', case=False, na=False)
                                          ].copy()
                
                # Exclude exercises already selected for this day
                cat_pool = cat_pool[~cat_pool['Exercise Name'].isin(temp_selected_names)]

                if cat_count > 0 and not cat_pool.empty:
                    
                    # Special logic to prevent Quad-only lower body days
                    if cat == 'Lower Body' and cat_count > 1:
                        selected_lower = []
                        
                        # 1. Target Glutes/Hamstrings
                        hamglute_pool = cat_pool[cat_pool['Target Region'].str.contains('Glute|Hamstring', case=False, na=False)].copy()
                        
                        if not hamglute_pool.empty:
                            selected_ex = hamglute_pool.sample(1, random_state=day_index + 1)
                            selected_lower.append(selected_ex)
                            cat_count -= 1
                            cat_pool = cat_pool[~cat_pool['Exercise Name'].isin(selected_ex['Exercise Name'])]
                            temp_selected_names.add(selected_ex['Exercise Name'].iloc[0])

                        # 2. Target Quads/General Lower Body
                        if cat_count > 0 and not cat_pool.empty:
                            selected_ex = cat_pool.sample(min(cat_count, len(cat_pool)), replace=False, random_state=day_index + 2)
                            selected_lower.append(selected_ex)
                            temp_selected_names.update(selected_ex['Exercise Name'].tolist())
                            
                        main_selection_list.extend(selected_lower)
                        
                    # Standard selection for Upper Body and Core
                    else:
                        selected_ex = cat_pool.sample(min(cat_count, len(cat_pool)), replace=False, random_state=day_index + i)
            ...
            if main_selection_list:
                ...
                while len(main_selection) < target_main_count:
                    ...
                    reused_selection = reusable_pool.sample(min(fill_needed, len(reusable_pool)), replace=False, random_state=day_index + len(main_selection))
    ...
    for day, df_plan in workout_plan.items():
        ...
        for part, group in df_plan.groupby('Part', sort=False):
            ...
            for idx, row in group.iterrows():
                ...
                # Title
                # The user's template shows the count (1., 2.) *outside* the exercise name but *inside* the instruction body.
                # However, to be strictly sequential with the example: <exercise count> Seated Wall Push-Ups 
                ...
                for step_content in steps:
                    ...
                    if step_content:
                        ...
                        # Use HTML/non-breaking spaces to force the exact indent and structure
                        # Use <br> to ensure a clean newline after the content, not just for the indent
