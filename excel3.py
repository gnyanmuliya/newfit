...
def load_exercise_data():
    ...
    # 4. Create Resistance Category (Crucial for balancing Push/Pull)
...
used_main_exercise_names = {} # Tracks {day: [names]} for repetition check
...
def generate_workout_plan(df_master: pd.DataFrame, profile: Dict[str, Any]) -> Dict[str, pd.DataFrame]:
    ...
    global used_main_exercise_names
    used_concept_groups = set() 
    used_main_exercise_names = {}
    ...
    target_cooldown_count = session_counts['cooldown_count'] # 2 (from DURATION_MAP)
    
    # Define required counts for balanced Full Body split (2 Push, 2 Pull, 3 Lower, 2 Core for 9 total)
    if target_main_count >= 9:
        if target_main_count == 9: RESISTANCE_TARGETS = {'Upper Body Push': 2, 'Upper Body Pull': 2, 'Lower Body': 3, 'Core': 2}
        elif target_main_count == 11: RESISTANCE_TARGETS = {'Upper Body Push': 3, 'Upper Body Pull': 3, 'Lower Body': 3, 'Core': 2}
    elif target_main_count == 7:
        RESISTANCE_TARGETS = {'Upper Body Push': 2, 'Upper Body Pull': 2, 'Lower Body': 2, 'Core': 1}
    else: # 5 exercises (min)
        RESISTANCE_TARGETS = {'Upper Body Push': 1, 'Upper Body Pull': 1, 'Lower Body': 2, 'Core': 1}
    ...
    # Determine the strict split focus for Day A and Day B
    # Day A: Lower Focus (e.g., 3 Lower / 2 Push / 2 Pull / 2 Core)
    # Day B: Upper Focus (e.g., 4 Upper / 1 Lower / 2 Core) - Adjusting targets for variety
    
    base_targets = RESISTANCE_TARGETS.copy()
    
    # Pre-define workout specific targets for the A-B-A split
    workout_targets = {}
    
    # Only if we have multiple workouts (A-B-A)
    if any(r in split_rules for r in ["Full-Body A", "Full-Body B"]):
        
        # Day A (Monday/Friday): Slightly Lower Body Focus (e.g., 3 Lower)
        targets_A = base_targets
        
        # Day B (Wednesday): Slightly Upper Body Focus (Shift 1 Lower exercise to 1 Upper Push)
        targets_B = base_targets.copy()
        targets_B['Lower Body'] = max(0, targets_B['Lower Body'] - 1)
        targets_B['Upper Body Push'] = targets_B.get('Upper Body Push', 0) + 1
        
        for rule in split_rules:
            if rule == "Full-Body A":
                workout_targets[rule] = targets_A
            elif rule == "Full-Body B":
                workout_targets[rule] = targets_B
            elif rule == "Active Recovery":
                workout_targets[rule] = {}
            else: # Default for non-split rules (e.g., 1 day)
                workout_targets[rule] = base_targets
    else:
        # Default for simple programs (1 or 2 days same)
        for rule in split_rules:
    ...
    for day_index, (day, rule) in enumerate(zip(training_days, split_rules)):
        ...
        # Reset concept group based on the rule (Day A concepts should be unique from Day B concepts)
        if rule == "Full-Body A":
            used_concept_groups = set()
            
        elif rule == "Full-Body B":
            # If Full-Body B is after A, use a fresh set of concepts
            # We don't want Day B to overlap with Day A's concepts
            used_concept_groups = set(used_concept_groups.copy())
        ...
        warmup_pool_full = df_working[df_working['Category'].str.contains('Warm-Up|Cardio|Mobility', na=False) & 
                                      ~df_working['Category'].str.contains('Strength|Resistance', na=False)].copy() 
        
        # Exclude concepts used in the Main Workout section of previous days
        warmup_pool = warmup_pool_full[~warmup_pool_full['Concept_Group'].isin(used_concept_groups)].copy()
        if warmup_pool.empty: warmup_pool = warmup_pool_full.copy()
        ...
        warmup_selection['RPE'] = warmup_selection['RPE'].apply(lambda x: re.sub(r'(\d+)-(\d+)', lambda m: f"{min(2, int(m.group(1)))}-{min(4, int(m.group(2)))}", str(x)))


        # 2. Main Workout (Strict Balanced Selection)
        if rule == "Active Recovery":
            main_selection = pd.DataFrame()
        else:
            main_pool = df_working[df_working['Resistance_Category'].isin(current_targets.keys())].copy()
            
            # CRITICAL ACCURACY FIX: Hypertrophy/Strength Enforcement
            if profile['primary_goal'] in ["Muscle Gain (Hypertrophy)", "Strength Gain"]:
                
                def is_high_rpe(rpe_str):
                ...
                main_pool = main_pool[main_pool['RPE'].apply(is_high_rpe)].copy()
                
                # Enforce hypertrophy parameters (Sets 3+, Reps 8-15, RPE 7-9)
            ...
            if main_pool.empty:
                 st.error(f"❌ ERROR: Cannot build Hypertrophy plan for {day}. No exercises matched RPE 7+ requirements using available equipment.")
                 return {"Error": pd.DataFrame()}

            main_selection_list = []
            
            # Temporary storage of used names for THIS day
            ...
            for res_cat, count in current_targets.items():
                ...
                # Filter out concepts used in previous days AND names used earlier in THIS day
                cat_pool = cat_pool[~cat_pool['Concept_Group'].isin(used_concept_groups)]
            ...
            if main_selection_list:
                main_selection = pd.concat([ex for ex in main_selection_list if not ex.empty])
                
                # Update the global set of used concepts
                used_concept_groups.update(main_selection['Concept_Group'].tolist())
                
                # Final volume check: If target not met (due to concept exhaustion), reuse concepts *if allowed*
                while len(main_selection) < target_main_count:
                    fill_needed = target_main_count - len(main_selection)
                    
                    # Reusable pool excludes concepts already selected for THIS week
                    reusable_pool = main_pool[~main_pool['Concept_Group'].isin(used_concept_groups)] 
                    reusable_pool = reusable_pool[~reusable_pool['Exercise Name'].isin(main_selection['Exercise Name'])] # Exclude current day's selection
        ...
        # 3. Cool-Down (Always 2-3 if possible)
        cooldown_pool_full = df_working[df_working['Category'].str.contains('Cool-Down|Stretch|Mobility', na=False) &
                                        ~df_working['Category'].str.contains('Strength|Resistance', na=False)].copy() 

        # Exclude concepts used in the Main Workout section
        cooldown_pool = cooldown_pool_full[~cooldown_pool_full['Concept_Group'].isin(used_concept_groups)].copy()
        if cooldown_pool.empty: cooldown_pool = cooldown_pool_full.copy() # Fallback
