    ...
    def generate_workout_plan(
        ...
        try:
            system_prompt = self._build_system_prompt(
                user_profile, day_name, day_index, workout_category
            )
            ...
            headers = {
                ...
                # Note: If the above fails, try 'api-key': self.api_key (but this led to the 400 error)
            }
            ...
            if response.status_code != 200:
                ...
                raise Exception(f"API returned {response.status_code} with error: {response.text}")
            
            result = response.json()
            
            # Extract plan text
            plan_text = ""
            # The structure for Azure Mistral/Anthropic models usually returns text content in 'choices[0].message.content'
            # The structure below seems to be based on an assumption about Anthropic's specific non-standard JSON response.
            # I will assume the structure for Azure/OpenAI compatibility for this endpoint:
            try:
                plan_text = result['choices'][0]['message']['content']
            except (KeyError, IndexError):
                # Fallback to the original structure if the above fails
    ...
    if not st.session_state.fitness_plan_generated:
        ...
        if st.session_state.generation_in_progress:
            ...
            if all_success:
                ...
                st.warning("⚠️ Some plans used fallback mode. Please check your API key and configuration.")
