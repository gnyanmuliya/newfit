...
def _get_cal_reference(weight_kg: float) -> Dict[str, str]:
    """
    [MODIFIED] Calculates plausible Cal/min ranges and MET values based on user weight for the LLM prompt.
    This replaces the previous interpolation logic with a formulaic guide.
    """
    ...
    # Round Cal/min values and provide METs
    def format_guide(value, met):
        low = int(max(1, value * 0.95))
        high = int(value * 1.05)
        return f"{met} (Range: {low}-{high} Cal/min)"
    ...
    cal_per_rep_range = {
        "Moderate": f"{round(cal_per_rep_mod * 0.9, 2)}-{round(cal_per_rep_mod * 1.1, 2)}",
        "High": f"{round(cal_per_rep_high * 0.9, 2)}-{round(cal_per_rep_high * 1.1, 2)}",
    }

    return {
        "Low_Guide": format_guide(cal_per_min["Low"], 3.0),
        "Moderate_Guide": format_guide(cal_per_min["Moderate"], 5.0),
        "High_Guide": format_guide(cal_per_min["High"], 8.0),
...
- **CALORIE FORMULA (Kcal/min)**: $$(MET \times {calorie_ref['Weight_KG']} \times 3.5) / 200$$ 
- **MET REFERENCE GUIDES (CRITICAL):**
  - **Low Intensity (RPE 1-4, Warmup/Cooldown)**: Use MET 3.0 (Approximate range: **{calorie_ref['Low_Guide']}**)
  - **Moderate Intensity (RPE 5-7, Main Workout)**: Use MET 5.0 (Approximate range: **{calorie_ref['Moderate_Guide']}**)
  - **High Intensity (RPE 8-9, Cardio/Intervals)**: Use MET 8.0 (Approximate range: **{calorie_ref['High_Guide']}**)
- **CALORIE MANDATE:** The 'est_calories' value MUST be derived by applying the relevant MET value and user weight to the Calorie Formula above, then multiplying by the duration of the set (in minutes) and rounding to the nearest integer.
"""
        ...
        prompt_parts = [
                ...
                "warmup": [
                    {
                        ...
                        "equipment": "string",
                        "safety_cue": "string",
                        "est_calories": "Est: X Cal (CRITICAL: Must be calculated using the MET formula in Section 5.)" 
                    }
                ],
                "main_workout": [
                    {
            ...
            "   2. Upper Body Dynamic Stretch/Mobility (e.g., Arm Circles, Band Pull-Aparts). The duration for this should be treated as 30 seconds for calculation.",
            "   3. Lower Body Dynamic Stretch/Mobility (e.g., Leg Swings, Standing Hip Circles). The duration for this should be treated as 30 seconds for calculation.",
            ...
            f"  - **Warmup Cardio (Exercise 1):** The planned duration is FIXED at 90 seconds. Your 'reps' field should indicate a large rep count (e.g., 60-120) to maintain LLM compliance.",
