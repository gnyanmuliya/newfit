...
def _get_cal_reference(weight_kg: float) -> Dict[str, str]:
    """
    Calculates plausible Cal/min ranges and MET values based on user weight for the LLM prompt.
    This is the core for calorie accuracy using the LLM.
    """
...
class FitnessAdvisor:
    ...
    def _calculate_calorie_rate(self, exercise_name: str, weight_kg: float) -> tuple[float, str]:
        ...
        The rate returned here is a placeholder (1.0) because the *actual* rate used for logging
        must be derived from the LLM's planned calorie output divided by the planned units.
        """
    ...
    def _calculate_total_estimated_calories(self, exercise_name: str, sets_str: str, reps_str: str, rest_str: Optional[str], hold_duration: Optional[str], weight_kg: float) -> str:
        """
        [DEPRECATED] This function now only provides a fallback placeholder string.
        The real calculation logic must be done by the LLM as requested.
        """
        return "Est: 15 Cal" 
...
"""
        ...
        prompt_parts = [
            ...
            f"  - **Warmup Cardio (Exercise 1):** The planned duration is FIXED at **90 seconds** (1.5 minutes). Your 'reps' field should indicate a large rep count (e.g., 60-120) to maintain LLM compliance.",
...
def initialize_session_state():
    ...
    # FIX: Initialize the specific_avoidance key in the user_profile structure itself
    # This ensures no KeyError is thrown when trying to read default values
    if 'specific_avoidance' not in st.session_state.user_profile:
        st.session_state.user_profile['specific_avoidance'] = "None"
    
    # Store the radio button selection state globally, outside the form
    if 'avoid_advised_state' not in st.session_state:
        st.session_state.avoid_advised_state = "No"
...
def main():
    ...
    # 1. Initialize radio state based on profile on the first run of main()
    # NOTE: This state is no longer strictly needed for controlling visibility
    # in the streamlined approach, but we keep it initialized for clarity.
    if 'avoid_advised_state' not in st.session_state:
        st.session_state.avoid_advised_state = "No"
    ...
    with st.form("fitness_form"):
        ...
        # This replaces the radio button and conditional rendering with a single text area.
