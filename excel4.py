...
def load_exercise_data():
    ...
    try:
        ...
        # This assumes the user has uploaded or placed the CSV file alongside the script.
...
def filter_and_select_exercises(df: pd.DataFrame, profile: Dict[str, Any]) -> pd.DataFrame:
    ...
    (Updated to adjust RPE filter and Equipment Prioritization for robustness)
    """
...
def generate_workout_plan(df_master: pd.DataFrame, profile: Dict[str, Any]) -> Dict[str, pd.DataFrame]:
    ...
    Implements Tiered Selection and Balance Guarantees (90%+ accuracy goal).
    """
    ...
    if filtered_df.empty:
        # This error message is essential when the pool is empty
    ...
    for day_index, (day, rule) in enumerate(zip(training_days, split_rules)):
        ...
        if rule == "Full-Body A":
            filter_concepts = used_concept_groups_B # Filter out concepts used on the opposite day (B)
            track_concepts = used_concept_groups_A # Track concepts used today (A)
        elif rule == "Full-Body B":
            filter_concepts = used_concept_groups_A # Filter out concepts used on the opposite day (A)
            track_concepts = used_concept_groups_B # Track concepts used today (B)
        else:
            # For non A/B splits, filter all used concepts
            filter_concepts = used_concept_groups_A.union(used_concept_groups_B) 
            track_concepts = used_concept_groups_A # Use A set as default tracker
            
        # Define Resistance Categories (Push, Pull, Lower, Core) to exclude from Warmup/Cooldown
        RESISTANCE_CATS = ['Upper Body Push', 'Upper Body Pull', 'Lower Body', 'Core']

        # 1. Warm-up (Always 3 if possible)
        # FIX: Strictly exclude resistance exercises from warm-up/cool-down pools
        warmup_pool_full = df_working[~df_working['Resistance_Category'].isin(RESISTANCE_CATS)].copy() 
        
        # Exclude concepts used in the Main Workout section of previous days (Tier 1 variety)
        warmup_pool = warmup_pool_full[~warmup_pool_full['Concept_Group'].isin(filter_concepts)].copy()
        if warmup_pool.empty: warmup_pool = warmup_pool_full.copy() # Fallback for exhausted pool
        ...
        # FIX: Ensure RPE for Warm-up is always low (1-4)
        ...
        if rule == "Active Recovery":
            ...
            # CRITICAL HYPERTROPHY PARAMETER ENFORCEMENT (Addressing 4 & 6)
            if profile['primary_goal'] in ["Muscle Gain (Hypertrophy)", "Strength Gain"]:
            ...
            if main_pool.empty:
                 # This check should now be less likely to trigger due to the relaxed filtering in step 6 above.
                 # Re-check needed here, if main_pool is empty, return an error now.
                 st.error(f"❌ ERROR: Cannot build main workout for {day}. No resistance exercises matched your strict profile criteria (Equipment, Level, RPE 7+).")
                 return {"Error": pd.DataFrame()}

            main_selection_list = []
            
            # Temporary storage of used names for THIS day (Ensures strict daily uniqueness)
            current_day_names = set() 
            
            # --- Tier 1 Selection (Unique Concept + Unique Name) ---
            for res_cat, count in current_targets.items():
                if count == 0: continue
                
                cat_pool = main_pool[main_pool['Resistance_Category'] == res_cat].copy()
                
                # Filter out concepts used in the opposite day's tracker set (A or B)
                cat_pool = cat_pool[~cat_pool['Concept_Group'].isin(filter_concepts)]
                
                # Filter out names used earlier in THIS day (Strict Daily Uniqueness)
            ...
            if main_selection_list:
                main_selection = pd.concat(main_selection_list)
                
                # Update the rule-specific tracker set (used_concept_groups_A or B) with Tier 1 selections
                track_concepts.update(main_selection['Concept_Group'].tolist())
                
                
                # --- Tier 2: Backfill for Volume & Balance (Guaranteed Balance Backfill) ---
                
                current_counts = main_selection['Resistance_Category'].value_counts().to_dict()
                
                for res_cat, target_count in current_targets.items():
                    current_count = current_counts.get(res_cat, 0)
                    fill_needed_for_balance = target_count - current_count
                    
                    if fill_needed_for_balance > 0:
                        
                        # Relaxation: Use exercises from the full main_pool for this category
                        cat_pool_relaxed = main_pool[main_pool['Resistance_Category'] == res_cat].copy()
                        
                        # Still filter out names used in THIS day (Strict Daily Uniqueness)
                        cat_pool_relaxed = cat_pool_relaxed[~cat_pool_relaxed['Exercise Name'].isin(current_day_names)]
                        
                        if cat_pool_relaxed.empty: continue
                        
                        # Select only what is needed to hit the target count for this category
                        selection_n = min(fill_needed_for_balance, len(cat_pool_relaxed))
                        
                        seed_val = abs(day_index + hash(res_cat) + 1000) % (2**32 - 1)
                        reused_selection = cat_pool_relaxed.sample(selection_n, replace=False, random_state=seed_val)

                        # Update the selection and daily uniqueness tracker
                        main_selection = pd.concat([main_selection, reused_selection]).drop_duplicates(subset=['Exercise Name'])
                        current_day_names.update(reused_selection['Exercise Name'].tolist())
                
                
                # --- Tier 3: Final Volume Fill (If still below target, fill with any remaining unique name) ---
                while len(main_selection) < target_main_count:
                    fill_needed = target_main_count - len(main_selection)
                    
                    # Full Reusable Pool: exclude only names already selected for the current day
                ...
                main_selection = pd.DataFrame() # Ensure it's empty if nothing was selected
                
        # 3. Cool-Down (Always 2-3 if possible)
        # FIX: Strictly exclude resistance exercises from warm-up/cool-down pools
        cooldown_pool_full = df_working[~df_working['Resistance_Category'].isin(RESISTANCE_CATS)].copy() 

        # Exclude concepts used in the Main Workout section
        cooldown_pool = cooldown_pool_full[~cooldown_pool_full['Concept_Group'].isin(track_concepts)].copy()
        ...
        # FIX: Ensure RPE for Cool-down is always low (1-3)
