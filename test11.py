...
class FitnessAdvisor:
    ...
    def _build_system_prompt(
        ...
        The prompt structure is: Persona -> JSON Schema -> Rules.
        """
        ...
        # Rep Range Safety Adjustment (FIX 5)
        ...
        if previous_training_days and previous_plans:
            days_to_check = previous_training_days[-2:] # Look at last 2 days
            for day_key in days_to_check:
                ...
                if plan_data and plan_data.get('success') and 'plan_json' in plan_data and plan_data['plan_json']:
                    ...
                    day_exercises = [ex.get('name', '').strip() for ex in all_exercises if ex.get('name')]
                    exercises_to_avoid.update(day_exercises)
        ...
        if medical_conditions and medical_conditions != ["None"]:
            for condition in medical_conditions:
                if condition != "None":
                    ...
                    medical_restrictions.append(f"CONDITION: {condition}. MUST AVOID: {cond_data.get('contraindicated', 'High-risk movements')}. PRIORITIZE: {cond_data.get('modified_safer', 'Low-impact alternatives')}.")
        ...
        if fitness_level in ["Level 1 – Assisted / Low Function", "Level 2 – Beginner Functional"] or bmi > 30 or age >= 50:
            advanced_avoid_exercises.extend([
                "Full Lunges", "Reverse Lunges (unassisted)", "Bicycle Crunch", "Leg Swings (Dynamic/Unsupported)", 
                "Crunches (any variation)", "Leg Raises", "Burpees", "Box Jumps"
            ])
            
        if advanced_avoid_exercises:
             medical_restrictions.append(f"SAFETY OVERRIDE: Given Age/BMI/Level, also AVOID: {', '.join(set(advanced_avoid_exercises))}. Prioritize Seated/Supported/Low-Impact variations.")


        # --- RULE INJECTION - STRUCTURE (Section 4) ---
        
        # Required Main Workout Categories (FIX 2 & 6)
        required_structure = ""
        ...
        prompt_parts = [
            "You are FriskaAI, an ACSM-CEP certified clinical exercise physiologist.",
            ...
            f"- Strictly use only these equipment options: **{allowed_equipment}**",
            f"- Exercises to avoid (from last 2 sessions - NO REPETITION in Warmup/Main/Cooldown): **{', '.join(exercises_to_avoid_list)}**",
            f"- Fitness level constraints: **{fitness_level_rules}**",
            f"- Medical and Safety Restrictions: {' | '.join(medical_restrictions) if medical_restrictions else 'None.'}",
            f"- Physical limitations: **{user_profile.get('physical_limitations', 'None')}**",
            "",
            "# 4. REQUIRED EXERCISE STRUCTURE",
            f"- Warmup: exactly 3 exercises (Total duration MUST be 5-7 minutes, FIX 4)",
            f"- Main workout: exactly {max_main_exercises} exercises (Volume reduced for safety/level)",
